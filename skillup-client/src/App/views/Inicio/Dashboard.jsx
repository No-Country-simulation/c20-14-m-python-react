...
const Dashboard = () => {
    const dashboardCards = CardsData.filter(card => card.id === 3 || card.id === 4 || card.id === 7);

    return (
        <Container fluid className="cardContainer">
             <Row>
            <Col><h2 className={css.heroText_ppal}>Dashboard</h2></Col>               
            </Row>
                ...
                {dashboardCards.map((card, index) => (
                    <Col xs={12} md={6} lg={4} key={index} className={css.cardCol}>
                        <Card className="mb-4">
                            ...
                            <Badge className="cardBadge" bg="primary">{card.hours} horas</Badge>
                            </div>
